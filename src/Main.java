...
public class Main {
    ...
    public static void main (String[] args){
       ...
       System.out.println("");
       phi = phi*Math.PI/180; // on traduit phi en radians pour les fonction cos et sin
           ...
           if(Math.sqrt(Math.pow(x,2)+Math.pow(y,2))>a1+a2+d4) // on vérifie que la branche est accessible par la norme
           {
               ...
               if (phi > 360) // si après avoir faitt un tour aucune valeur de phi ne va alors on renvoie cette position comme non accessible
               {
                   ...
                   double xobtenu=d4*Math.sin(phi*Math.PI/180)+a2*Math.cos((theta1+theta2)*Math.PI/180)+a1*Math.cos(theta1*Math.PI/180);
                   double yobtenu=-d4*Math.cos(phi*Math.PI/180)+a2*Math.sin((theta1+theta2)*Math.PI/180)+a1*Math.sin(theta1*Math.PI/180);
